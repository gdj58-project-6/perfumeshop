...
public class CommentService {
	...
	// 상품문의
	public ArrayList<HashMap<String, Object>> getGoodsQuestionListByAdmin(int goodsCurrentPage, int goodsRowPerPage) {
		// 객체 초기화
		ArrayList<HashMap<String, Object>> list =  null;
		// 드라이버 초기화
		Connection conn = null;
		
		int beginRow = (goodsCurrentPage - 1) * goodsRowPerPage;
		...
		try {
			...
			// dao 호출
			this.commentDao = new CommentDao();
			list = commentDao.selectGoodsQuestionListByAdmin(conn, beginRow, goodsRowPerPage);
			// 커밋
			conn.commit();
		} catch(Exception e) {
			e.printStackTrace();
