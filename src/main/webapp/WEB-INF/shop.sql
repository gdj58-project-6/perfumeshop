...
CREATE TABLE IF NOT EXISTS `customer` (
  ...
  `point` int(11) NOT NULL DEFAULT 10 COMMENT '반정규화',
  `auth_code` int(11) NOT NULL DEFAULT 0,
  `createdate` datetime NOT NULL DEFAULT current_timestamp(),
  PRIMARY KEY (`customer_code`),
  UNIQUE KEY `customer_id` (`customer_id`),
  KEY `FK_customer_auth_info` (`auth_code`),
  CONSTRAINT `FK_customer_auth_info` FOREIGN KEY (`auth_code`) REFERENCES `auth_info` (`auth_code`) ON DELETE NO ACTION ON UPDATE NO ACTION
...
CREATE TABLE IF NOT EXISTS `emp` (
  ...
  `auth_code` int(11) NOT NULL DEFAULT 4,
  `createdate` datetime NOT NULL DEFAULT current_timestamp(),
